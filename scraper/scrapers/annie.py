...
def scrape_annie(ceremony_number):
    ...
    if not best_feature_th:
        ...
        if section_header:
            ...
            for elem in following_elements:
                ...
                # Skip "Winner:" or "Nominees:" labels
                if elem.name == 'p' and ('Winner' in text or 'Nominee' in text):
                    continue
                
                # NEW: First p that's NOT a label is the winner (e.g., "Rango - Paramount...")
                if elem.name == 'p' and not first_p_processed and text:
                    first_p_processed = True
                    # Extract film name from p text (before – or - separator)
                    film_name = text
                    if '–' in film_name:
                ...
                # Process ul lists for nominees
                if elem.name == 'ul':
                    for li in elem.find_all('li', recursive=False):
                        # Check if this list item is bold (winner - fallback)
                        ...
                        if film_name:
                            # If item is bold and no winner yet, it's the winner
                            if is_bold and not found_winner:
                                found_winner = film_name
                            else:
    ...
    # FALLBACK 1.5: Winner in bold before ul (28th Annie structure)
    # Structure: <b>Winner Film</b> followed by <ul> of nominees
    if not best_feature_th:
        for b in soup.find_all('b'):
            # Check if this bold contains a film (likely after "Best Animated Feature")
            preceding_text = ''
            prev_sibling = b.find_previous_sibling()
            if prev_sibling:
                preceding_text = prev_sibling.get_text() if hasattr(prev_sibling, 'get_text') else str(prev_sibling)
            
            # Also check if parent is a li and the li contains the winner
            parent_li = b.find_parent('li')
            if parent_li:
                # Check if next sibling of this li's parent ul is a ul with nominees
                parent_ul_of_winner = parent_li.find_parent('ul')
                
                # This is actually looking for a bold element right before a ul
                # Look for structure: <b>Winner</b> followed by <ul>nominees</ul>
                pass
            
            # Check for pattern: bold film followed by ul with nominees
            next_ul = b.find_next_sibling('ul')
            if next_ul:
                # Get winner from bold
                winner_text = b.get_text().strip()
                # Extract film name
                if '–' in winner_text:
                    winner_name = winner_text.split('–')[0].strip()
                elif ' - ' in winner_text:
                    winner_name = winner_text.split(' - ')[0].strip()
                else:
                ...
                for li in next_ul.find_all('li', recursive=False):
                    ...
                    if '–' in film_text:
                        ...
                        film_name = film_text
                    if film_name:
                        nominees_found.append(film_name)
                
                # If we found a reasonable structure (winner + nominees), use it
                if winner_name and len(nominees_found) >= 3:
                    results['best-film'].append({
                        'name': winner_name,
                        'awards': {'annie': 'Y'}
                    })
                    for nominee in nominees_found:
                        results['best-film'].append({
                            'name': nominee,
                            'awards': {'annie': 'X'}
                        })
                    ...
                    total = len(results['best-film'])
                    print(f"    Annie {ordinal_str}: Found {total} entries (Winner: 1, Nominees: {total - 1}) [bold+ul structure]")
                    return results
                ...
                for td in table.find_all('td'):
                    ...
                    if 'Best Animated Feature' in td_text:
                        ...
                        for line in lines:
                            ...
                            # Check for winner marker († or ‡ - both used on different pages)
                            is_winner = '†' in line or '‡' in line
                            
                            # Extract film name (before – or - that separates studio)
                            film_name = line.replace('†', '').replace('‡', '').strip()
                            if '–' in film_name:
                                film_name = film_name.split('–')[0].strip()
                            elif ' - ' in film_name:
                                film_name = film_name.split(' - ')[0].strip()
    ...
    # STRUCTURE 4: Winner as plain text before <ul> (e.g., 30th Annie)
    # The first text node before the <ul> is the winner
    if main_ul and not p_tag:
        # Get all text directly in td before the ul
        td_text = ''
        for child in td.children:
            if child == main_ul:
                break
            if hasattr(child, 'get_text'):
                td_text += child.get_text()
            elif isinstance(child, str):
                td_text += child
        
        td_text = td_text.strip()
        if td_text:
            # First line is the winner
            lines = [line.strip() for line in td_text.split('\n') if line.strip()]
            if lines:
                winner_text = lines[0]
                # Extract film name (before – or - separator)
                film_name = winner_text
                if '–' in film_name:
                    film_name = film_name.split('–')[0].strip()
                elif ' - ' in film_name:
                    film_name = film_name.split(' - ')[0].strip()
                ...
                for li in main_ul.find_all('li', recursive=False):
                    film_i = li.find('i')
                    if film_i:
                        fn = film_i.get_text().strip()
                    else:
                        fn = li.get_text().strip()
                ...
                total = len(results['best-film'])
                winners = len([f for f in results['best-film'] if f['awards']['annie'] == 'Y'])
                print(f"    Annie {ordinal_str}: Found {total} entries (Winner: {winners}, Nominees: {total - winners}) [text before ul]")
                return results
